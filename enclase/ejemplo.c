...
#define v 1  //la velocidad de la ecuacion de adveccion
#define cuadros 50 //numero de cuadros del gif
#define m1 0.06
;
double cond_in(float x);
...
printf("Haz elegido %s\n",opcion);
  ...
  for (int i = 0; i < t-1; i++) {
       for (int j = 1; j < n-1; j++) {
             malla[i+1][j]=malla[i][j]-0.5*ro*(pow(malla[i][j],2)-pow(malla[i][j-1],2));
       }
  }
  break;
  case 4:
...
if(v>=0){
      for (int i = 0; i < t-1; i++) {
           for (int j = 1; j < n-1; j++) {
                 malla[i+1][j]=malla[i][j]-ro*malla[i][j]*(malla[i][j]-malla[i][j-1]);
           }
      }
    }
    if(v<0){
