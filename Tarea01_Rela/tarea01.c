...
int main()
{
	...
	double x_ext[N], y_ext[N], z_ext[N];
     y_ext[1]=y[N+1];
     x_ext[1]=x[N+1];
		 z_ext[1]=z[N+1];
     for(i=1; i<=N+1; i++)
	...
	for(i=1; i <= N+1; i++)
	{
		k1 = h*f(x_ext[i], y_ext[i],z_ext[i]);
		l1 = h*g1(x_ext[i],y_ext[i],z_ext[i]);
		k2 = h*f(x_ext[i]+0.5*h, y_ext[i]+0.5*k1,z_ext[i]+0.5*l1);
		l2 = h*g1(x_ext[i]+0.5*h, y_ext[i]+0.5*k1,z_ext[i]+0.5*l1);
		k3 = h*f(x_ext[i]+0.5*h, y_ext[i]+0.5*k2, z_ext[i]+0.5*l2);
		l3 = h*g1(x_ext[i]+0.5*h, y_ext[i]+0.5*k2, z_ext[i]+0.5*l2);
		k4 = h*f(x_ext[i]+0.5*h, y_ext[i]+k3, z_ext[i]+l3);
		l4 = h*g1(x_ext[i]+0.5*h, y_ext[i]+k3, z_ext[i]+l3);

		y_ext[i+1] = y_ext[i] + 0.16666*(k1 + 2.0*k2 + 2.0*k3 + k4);
		z_ext[i+1] = z_ext[i] + 0.16666*(l1 + 2.0*l2 + 2.0*l3 + l4);
